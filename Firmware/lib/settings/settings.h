...
const char *mqttServer = "52.220.9.202";
const char *mqtt_server = "42c1410bece7497a99cfa15427041510.s2.eu.hivemq.cloud";
...
byte StrToHex(char str[]) { return (byte)strtol(str, 0, 16); }
byte toHexc(String s)
{
    ...
    tmp = s;
    xhex[tmp.length() + 2];
    tmp.toCharArray(xhex, tmp.length() + 2);
...
int encodeUTF8(char *dest, const char *src)
{
    int i = 0, j = 0;
    while (src[i])
    {
